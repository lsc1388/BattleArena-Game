...
class GameUI:
    ...
    def _draw_skill_cooldown(self, screen, player):
        ...
        # 計算技能冷卻剩餘時間
        skill_cooldown_total = 10000  # 10秒冷卻
        time_since_last_skill = current_time - player.last_skill_time

        if time_since_last_skill < skill_cooldown_total:
            cooldown_left = (skill_cooldown_total - time_since_last_skill) / 1000
            skill_text = f"技能: {cooldown_left:.1f}s"
            color = COLORS["red"]
        else:
            skill_text = "技能: 就緒"
            color = COLORS["green"]
